...
let totalMovies = 'Total de películas: ' + movies.length;
let moviesList = movies.map( movie => {
    return movie.title;
});
moviesList = moviesList.sort();
let mostAverage = movies.filter( movie => {
...
average = (average/mostAverage.length).toFixed(1);
	...
	switch (req.url) {
		...
		case '/':
            ...
            moviesList.forEach( (movie, index) => {
                return res.write((index+1) + '. ' + movie + '\n');
            });
            // Pié de página
		...
		case '/en-cartelera':
            ...
            movies.forEach( (movie, index) => {
                ...
                return res.write(movie.overview + '\n\n');
            });
			res.end();
            break;
        // Más votadas
