...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			movieList = movies.map( obj => {
				return obj.title;
			});
			movieList = movieList.sort();
			movieList.forEach( titles => {
				let title = titles + '\n';
				res.write(title);
			});
